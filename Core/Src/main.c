...
/* USER CODE BEGIN PV */
volatile uint8_t spi_dma_in_flight = 0;
volatile uint32_t spi_dma_success_count = 0;
volatile uint32_t spi_dma_busy_count = 0;
volatile uint32_t spi_dma_error_count = 0;
volatile uint32_t spi_last_error_code = 0;
...
uint32_t UsToTicks(uint32_t us) {
  ...
  double tick_ns = 1.0e9 / timer_freq;
  return (uint32_t)ceil(us * 1000. / tick_ns);
}
...
int main(void)
{
  ...
  // Set up TIM2's GPIO DMAs. On update, drive CSn low. On channel 2 compare,
  // drive CSn high.
  uint16_t csn_mask = GPIO_PIN_4;
  // HAL_DMA_Start(&hdma_tim2_up, (uint32_t)&csn_mask, (uint32_t)&GPIOA->BRR, 2);
  // HAL_DMA_Start(&hdma_tim2_ch2, (uint32_t)&csn_mask, (uint32_t)&GPIOA->BSRR, 2);

  // ch1 is purely used as a dummy transfer required to unblock the SPI transfers.
  static uint32_t dummy;
  hdma_tim2_ch1.XferCpltCallback = Tim2Ch1DmaComplete;
  HAL_DMA_Start_IT(&hdma_tim2_ch1, (uint32_t)&dummy, (uint32_t)&dummy, 1);

  // Compute the levels for the OC1 and OC2 channels.
  LL_TIM_OC_SetCompareCH1(TIM2, UsToTicks(500));

  uint32_t timer_freq = HAL_RCC_GetPCLK1Freq();
  double tick_ns = 1.0e9 / timer_freq;
  int nbits = 8;  // Number of bits in the SPI transfer.
  double bit_time_ns = 1.e-9 / 1e7; // 10 Mbps
  double ch2_cycle = UsToTicks(500) + ceil(bit_time_ns * nbits) + 50. / tick_ns;
  LL_TIM_OC_SetCompareCH2(TIM2, ch2_cycle);

  // Enable all the DMA requests for TIM2.
